...
contract("MulsiSig", (accounts)=> {
    ...
    it("should be able to receive ether", async()=> {
        ...
        await instance.sendTransaction({from: alice, value:web3.utils.toWei("1", "ether")});
    ...
    it("should be able to submit a transaction", async() => {
        await instance.submitTransaction(paul, web3.utils.toWei("0.5", "ether"), '0x6162636400000000000000000000000000000000000000000000000000000000',{from: alice});
        assert.equal(await instance.getTransactionCount(),1);
    });
    it("should be able to confirm a transaction", async() => {
        await instance.submitTransaction(paul, web3.utils.toWei("0.5", "ether"), '0x6162636400000000000000000000000000000000000000000000000000000000',{from: alice});
        await instance.confirmTransaction(0, {from: alice});
    ...
    it("should be able to execute a transaction", async()=> {
        ...
        await instance.submitTransaction(paul, web3.utils.toWei("0.5", "ether"), '0x6162636400000000000000000000000000000000000000000000000000000000',{from: alice});
        await instance.confirmTransaction(0, {from: alice});
        await instance.confirmTransaction(0, {from: bob});
        await instance.executeTransaction(0, {from: alice});
    ...
    it("only owner should be able to submit a transaction", async()=> {
        try {
            await instance.submitTransaction(paul, web3.utils.toWei("0.5", "ether"), '0x6162636400000000000000000000000000000000000000000000000000000000',{from: paul})
            assert(false);
        } catch (error) {
            assert(error);
        }
    ...
    it("only owner can execute a transaction", async()=> {
        await instance.submitTransaction(paul, web3.utils.toWei("0.5", "ether"), '0x6162636400000000000000000000000000000000000000000000000000000000',{from: alice});
        try {
            await instance.confirmTransaction(0, {from: paul})
            assert(false)
        } catch (error) {
            assert(error)
        }
    ...
    it("only owner can execute a transaction", async()=> {
        ...
        await instance.confirmTransaction(0, {from: bob});
        try {
            await instance.executeTransaction(0, {from: paul});
            assert(false)
        } catch(error) {
            assert(error)
        }
    ...
    it("only owner can revoke a transaction", async()=> {
        ...
        await instance.confirmTransaction(0, {from: bob});
        try {
            await instance.revokeConfirmation(0, {from: paul});
            assert(false)
        } catch(error) {
            assert(error)
        }
    ...
    it("cannot confirm a transaction that does not exists", async() => {
        await instance.submitTransaction(paul, web3.utils.toWei("0.5", "ether"), '0x6162636400000000000000000000000000000000000000000000000000000000',{from: alice});
        try {
            await instance.confirmTransaction(1, {from: alice});
            assert(false)
        } catch (error) {
            assert(error)
        }
    ...
    it("cannot revoke transaction that does not exists", async()=> {
        try {
            await instance.revokeConfirmation(0, {from: alice});
            assert(false)
        } catch (error) {
            assert(error)
        }
    ...
    // EVENTS
    // should emit an event when submit transaction
    // should emit an event when confirmTransaction
    // should emit an event when execute transaction
    // should emit an event when revoke transaction
